...
pipeline {
    ...
    environment {     // The environment block has a helper method credentials() defined which can be used to access pre-defined Credentials by their identifier in the Jenkins environment.
        ...
        DOCKER_REGISTRY_CREDENTIALS_ID = "docker-bot-test-knote" //change (provided via LP ... jenkins.bot.xxx and neeeds to be stored in jenkins credentials)
        DOCKER_IMAGE = "knote" //change (project name or something)
        JENKINS_VARIJABLA = "Vrijednost"
        JENKINS_VARIJABLA1 = "Vrijednost2"
        TRANSCRYPT_CREDENTIALS_ID = "transcrypt-knote" //change
